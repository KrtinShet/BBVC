...
const SingeType: React.FC<{}> = () => {
  ...
  const { write, data, isLoading: isContractLoading, isSuccess: isContractSuccess } = useContractWrite({
  ...
  useEffect(() => {
    if (isContractSuccess) {
      enqueueSnackbar('Certificate Successfully Uploaded', { variant: 'success' })
      ...
      <Stack spacing={3}>
        <Grid container spacing={5} mt={3}>
          <Grid item xs={12} md={6}>
            <Stack spacing={3}>
              <TextField name="Address" fullWidth label="Address" value={address} onChange={e => setAddress(e.target.value)} />
              <Box>
